...
def start_callback(msg):
    '''
    Callback method for Logical Camera Subscription
    '''
    if msg.models:
        MoveGroupPythonInteface.package_type = msg.models[0].type
...
def main():
    ...
    #Adding delay so the node waits for the models to spawn in Gazebo and Rviz
    rospy.sleep(30)
    try:
        command_ur5 = MoveGroupPythonInteface()
        #Now we create a client object to control vacuum gripper
        #and conveyer belt through ROS Service
        vacuum_grip = rospy.ServiceProxy('/eyrc/vb/ur5_1/activate_vacuum_gripper', vacuumGripper)
        conveyer_move = rospy.ServiceProxy('/eyrc/vb/conveyor/set_power', conveyorBeltPowerMsg)
        #Starting the Conveyer Belt
        conveyer_state = conveyer_move(power=25)
        #Going to pick the Red Box
        command_ur5.go_to_joint_state(2.967, -0.8203, 1.2566, -2.0769, -1.5882, 0.0)
        rospy.sleep(3.3)
        conveyer_state = conveyer_move(power=0)
        #Checking the Feed from Logical Camera and gripping the Red Box
        rospy.Subscriber('/eyrc/vb/logical_camera_2', LogicalCameraImage, start_callback)
        result = vacuum_grip(activate_vacuum_gripper=True)
        #Going to place the Red Box in the Red Bin
        command_ur5.go_to_joint_state(1.117, -0.733, 0.5759, -1.4486, -1.5184, 0.0)
        conveyer_state = conveyer_move(power=25)
        result = vacuum_grip(activate_vacuum_gripper=False)
        #Going to pick the Green Box
        command_ur5.go_to_joint_state(2.967, -1.117, 1.7278, -2.234, -1.5882, 0.0)
        rospy.sleep(1)
        conveyer_state = conveyer_move(power=0)
        #Checking the Feed from Logical Camera and gripping the Green Box
        rospy.Subscriber('/eyrc/vb/logical_camera_2', LogicalCameraImage, start_callback)
        result = vacuum_grip(activate_vacuum_gripper=True)
        #Going to place the Green Box in the Green Bin
        command_ur5.go_to_joint_state(0.0349, -0.4886, -0.1047, -0.9075, -1.5533, -1.3613)
        conveyer_state = conveyer_move(power=25)
        result = vacuum_grip(activate_vacuum_gripper=False)
        #Going to pick the Blue Box
        command_ur5.go_to_joint_state(3.0543, -0.4886, 0.5934, -1.7627, -1.6057, 0.0)
        rospy.sleep(0.5)
        conveyer_state = conveyer_move(power=0)
        #Checking the Feed from Logical Camera and gripping the Blue Box
        rospy.Subscriber('/eyrc/vb/logical_camera_2', LogicalCameraImage, start_callback)
        result = vacuum_grip(activate_vacuum_gripper=True)
        #Going to place the Blue Box in the Blue Bin
        command_ur5.go_to_joint_state(1.5882, -1.6755, -2.0769, -0.9773, 1.5184, 0.0)
        result = vacuum_grip(activate_vacuum_gripper=False)
        command_ur5.go_to_joint_state(2.967, -0.8203, 1.2566, -2.0769, -1.5882, 0.0)
